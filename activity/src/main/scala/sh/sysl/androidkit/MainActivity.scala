...
class MainActivity extends SDLActivity:
  ...
  /** Defined in `main.sysl`, not here.
    ...
    * `sh$sysl$androidkit$MainActivity$$nativeSetSystemBars` so the inner class can see it — and JNI
    * then looks for `Java_..._sh_00024sysl_00024androidkit_00024MainActivity_00024_00024nativeSetSystemBars`,
      ...
      new View.OnApplyWindowInsetsListener:
        def onApplyWindowInsets(v: View, insets: WindowInsets): WindowInsets =
          if Build.VERSION.SDK_INT >= Build.VERSION_CODES.R then
            ...
            Log.i("androidkit", s"system bars: $named")
